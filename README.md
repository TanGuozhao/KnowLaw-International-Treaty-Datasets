...
当前数据文件位于 `data/json/`，每个 JSON 文件对应一份法律文件。文件采用统一的文档树结构，包含 `meta` 元数据和 `children` 正文结构节点。
...
```powershell
python plugins/treaty-ast-integrity-checker/scripts/check_treaty_ast.py "data/json/巴黎协定.json"
```
